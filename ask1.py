...
def create_user():
    ...
    if users.find({"username":data["username"]} or {"password":data["password"]}).count() == 0:
...
def get_student():
    ...
    if students.find({"email":data["email"]}).count() == 0 :
            if uuid == False:
                return Response('the user has not been aythenticated', status=401, mimetype='application/json')
            else:
                return Response('The user haw been authonticated', mimetype='application/json')
        ...
        return Response('No student found with that email '+ email +' was found', status=400, mimetype='application/json')
...
def get_students_thirty():
    ...
    uuid = request.headers.get('authorization')
    if students.find({"yearOfBirth":data["yearOfBirth"]}).count() == 30 :
            if uuid == False:
                return Response('the user has not been aythenticated', status=401, mimetype='application/json')
            else:
                return Response('The user haw been authonticated', mimetype='application/json')
        
        # Η παρακάτω εντολή χρησιμοποιείται μόνο στη περίπτωση επιτυχούς αναζήτησης φοιτητών (δηλ. υπάρχουν φοιτητές που είναι 30 ετών).
